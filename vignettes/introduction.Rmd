...
Then, you can specify the temporal period by a character string. And, since we use parallel procesing to process the data, you may specify the number of cores you want to use for parallel.

```{r mpc-focal-dates, eval = FALSE}
# Use the format of "start_date/end_date"
focalDates <- "2010-01-01/2022-12-31"

# Number of cores
ncores <- 10
```

To get the data, you can use `GetEvi2PointTs()` function (it's going to take a while for the function to prepare the data, depending on how long the requested temperal period is and the internet speed).
