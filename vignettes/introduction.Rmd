...
library(data.table)
sample_data <- fread("gee/sampleData.csv")
```
...
single_ts <- sample_data[id == 3, .(date, evi2)]
...
Now, we can use the `blsp` package as usual. This code chunk might run for several minutes depending on how many years are in the time series. Note that to get a better fit, you could assign weights to observations to indicate observation quality (assigning weights was described above). Here we omit the step to show the worst case.
